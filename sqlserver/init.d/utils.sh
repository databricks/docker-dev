...
cli_replicant() {
    /opt/mssql-tools/bin/sqlcmd -h -1 -t 1 -U ${REPLICANT_USER:-replicant} -P ${REPLICANT_PW:-"Replicant#123"} -d master
}

cli_root() {
    /opt/mssql-tools/bin/sqlcmd -h -1 -t 1 -U sa -P $MSSQL_SA_PASSWORD 
}
