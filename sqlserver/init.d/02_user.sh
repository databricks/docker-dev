...
src_user() {
    ...
    heredoc_file ${PROG_DIR}/lib/02_src_root.sql | tee ${INITDB_LOG_DIR}/02_src_root.sql 
    cli_root < ${INITDB_LOG_DIR}/02_src_root.sql
...
if [ -f ${INITDB_LOG_DIR}/02_user.txt ]; then
    ...
    dst_user | tee -a ${INITDB_LOG_DIR}/02_user.txt
    if [[ "${ROLE^^}" = "SRC" ]]; then
        src_user | tee -a ${INITDB_LOG_DIR}/02_user.txt
    fi
