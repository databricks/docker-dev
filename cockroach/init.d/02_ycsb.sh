...
CREATE INDEX IF NOT EXISTS YCSBSPARSE_TS ON YCSBSPARSE(TS);
...
CREATE TRIGGER UPDATE_TS_ON_YCSBSPARSE BEFORE UPDATE ON YCSBSPARSE FOR EACH ROW EXECUTE PROCEDURE UPDATE_TS();
EOF
}
...
ycsb_load() {
    ...
    seq 0 $(( 1000000*${SIZE_FACTOR:-1} - 1 )) | \
        ...
        -c "copy YCSBSPARSE (ycsb_key) from STDIN" 
