...
create_user() {
    ...
    CREATE USER IF NOT EXISTS '${DB_ARC_USER}${SIZE_FACTOR}'@'%' IDENTIFIED WITH mysql_native_password BY '${DB_ARC_PW}';
    CREATE USER IF NOT EXISTS '${DB_ARC_USER}${SIZE_FACTOR}'@'127.0.0.1' IDENTIFIED WITH mysql_native_password BY '${DB_ARC_PW}';
