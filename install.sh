...
pullDockerDev() {
    if [[ -z "$DOCKERDEV_INSTALL" ]]; then
        ...
        echo "git clone https://github.com/databricks/${DOCKERDEV_NAME} ."
        git clone https://github.com/databricks/${DOCKERDEV_NAME} . >/tmp/install.$$ 2>&1
        if [[ "$?" != 0 ]]; then 
            cat /tmp/install.$$
            abort "git clone https://github.com/databricks/${DOCKERDEV_NAME} failed."
        fi
